    ...
    <el-row class="data-warp">
      <el-col :xs="24" :sm="24" :md="24" :lg="11" :xl="11">
        <div class="dailyData-display">
          <ul class="dailyData-item">
            <li class="data-title" />
            <li class="data-title">{{ $t('theData.theData21') }}</li>    <!-- 进件人数 -->
      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="11" :xl="11">
        <div class="dailyData-display">
          <ul class="dailyData-item">
            <li class="data-title" />
            <li class="data-title">{{ $t('theData.theDate24') }}</li>    <!-- 总还款笔数 -->
            <li class="data-title">{{ $t('theData.theDate25') }}</li>    <!-- 到期笔数 -->
            <li class="data-title">{{ $t('theData.theDate26') }}</li>    <!-- 到期未还笔数 -->
            <li class="data-title">{{ $t('theData.theDate27') }}</li>    <!-- 未还比率(%) -->
          </ul>
          <ul v-for="(item, i) in tabelData" :key="item.date" class="dailyData-item">
            <li class="data-time" :style="{ color: (i === 0 ? 'red' : '') }">
              {{ i === 0 ? $t('theData.theData22') : $t('theData.theData23') }}
...
export default {
  ...
  methods: {
    ...
    async getMarketInfo() {
      ...
      this.tabelData.push(this.homeData.dailyStatVos[0], this.homeData.dailyStatVos[1])
    ...
    chartB1Filter(xData, yData) {
      ...
      this.b1Data.yData = [
        {
          y: [],
          name: this.$t('theData.theData21') /* 进件人数 */
        },
        {
          y: [],
    ...
    chartB2Filter(xData, yData) {
      ...
      this.b2Data.yData = [
        {
          y: [],
          name: this.$t('theData.theDate24') /* 总还款笔数 */
        },
        {
          y: [],
          name: this.$t('theData.theDate25') /* 到期笔数 */
        },
        {
          y: [],
          name: this.$t('theData.theDate26') /* 到期未还笔数 */
        },
        {
          y: [],
          name: this.$t('theData.theDate27') /* 未还比率(%) */
        }
      ]
      ...
      this.b2Data.select = {
        ...
        data: [
          this.$t('theData.theDate24'), /* 总还款笔数 */
          this.$t('theData.theDate25'), /* 到期笔数 */
          this.$t('theData.theDate26'), /* 到期未还笔数 */
          this.$t('theData.theDate27') /* 未还比率(%) */
        ]
      }
      xData.forEach(item => {
        const i = yData.findIndex(val => val.date === item)
