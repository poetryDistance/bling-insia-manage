  ...
  <div class="main-box">
    ...
    <el-form ref="listQueryForm" :model="listQuery" size="mini" label-width="auto" inline>
      ...
      <el-form-item prop="aadhaarNo" :label="$t('borrow.borrow1') + ':'"> <!-- 身份证号 -->
        <el-input v-model="listQuery.aadhaarNo" :placeholder="$t('borrow.borrow2')" /> <!-- 请输入身份证号 -->
      </el-form-item>
    ...
    >
      ...
      <el-table-column :label="$t('borrow.borrow1')" prop="aadhaarNo" align="center" min-width="120px" /> <!-- 身份证号 -->
...
export default {
  ...
  methods: {
    ...
    handleListQuery() {
      ...
      if (tmp.periodStatus === undefined) {
        tmp.status = [70, 80, 90]
      } else {
        tmp.status = [tmp.periodStatus]
      }
      return tmp
    },
    // 查询获取订单信息
