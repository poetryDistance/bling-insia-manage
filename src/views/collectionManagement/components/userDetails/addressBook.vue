<template>
  ...
  <div style="padding-bottom: 50px;">
    ...
    >
      <el-table-column prop="contactDisplayName" :label="$t('common.common13')" align="center" /> <!-- 姓名 -->
      <el-table-column prop="timesContacted" :label="$t('loanMarket.loanMarket51')" align="center" /> <!-- 联系次数 -->
      <el-table-column prop="number" :label="$t('common.common28')" align="center"> <!-- 手机号 -->
        <template slot-scope="{ row }">
          <div style="cursor: pointer;" @click="openAdd(row.name, row.number)">{{ row.number }}</div>
        </template>
      </el-table-column>
      <el-table-column
        :label="$t('financial.financial28')"
        prop="lastTimeContacted"
        align="center"
        min-width="150px"
      > <!-- 与联系⼈最后联系时间 -->
        <template slot-scope="{ row }">
          {{ row.lastTimeContacted | formatterTimeYMDHMS }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('collect.collect149')" prop="upTime" align="center" min-width="150px"> <!-- 更新时间 -->
        <template slot-scope="{ row }">
          {{ parseInt(row.upTime) | formatterTimeYMDHMS }}
        </template>
      </el-table-column>
    </el-table>
    <pagination
...
export default {
  ...
  methods: {
    ...
    async getCustomerContacts() {
      ...
      console.log(this.$route.params.borrowIdRes)
      this.listQuery.borrowIdRes = this.$route.params.borrowIdRes
      this.listQuery.customerIdRes = this.$route.params.userIdRes
    ...
    // 通讯录假分页
    getContactsLists() {
      this.contactsData = this.contactsLists && this.contactsLists.slice((this.listQuery.page - 1) * this.listQuery.size, (this.listQuery.page - 1) * this.listQuery.size + this.listQuery.size)
    },
    // 获取通话记录信息
    async getCustomerCallRecords() {
      this.listLoading = true
      const res = await getCustomerCallRecords({
        borrowIdRes: this.$route.params.borrowIdRes,
        customerIdRes: this.$route.params.userIdRes
      })
      this.listLoading = false
