<template>
  <div>
    ...
    >
      <el-table-column v-if="isBooked" :label="$t('sysManage.sysManage23')" align="center" min-width="90px"> <!-- 操作 -->
...
export default {
  ...
  mounted() {
    setTimeout(() => {
      this.isUserDetails = this.sessionData('get', 'isUserDetails') ? this.sessionData('get', 'isUserDetails')[this.commonData.userId] : ''
  ...
  methods: {
    ...
    async getBorrowPeriods() {
      let res = null
      if (this.isLoan || this.isUserDetails === '2' || this.isUserDetails === '6') {
        this.periodsQuery.borrowIdRes = this.commonData.borrowIdRes
        res = await getBorrowPeriods(this.periodsQuery)
      } else if (this.isUserDetails === '1' || this.isUserDetails === '3') {
        this.periodsQuery.borrowPeriodIdRes = this.commonData.periodIdRes
        res = await getBorrowPeriods(this.periodsQuery)
      }
      if (res) {
