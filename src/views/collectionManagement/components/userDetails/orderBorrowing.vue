<template>
  <div>
    ...
    >
      <el-table-column v-if="isUserDetails === '4'" type="expand" :label="$t('userManage.userManage17')"> <!-- 分期 -->
      ...
      <el-table-column :label="$t('collect.collect2')" prop="id" align="center" min-width="120px" /> <!-- 订单编号 -->
      ...
      <el-table-column v-if="permission.isAdmin" key="modelResultTime" :label="$t('collect.collect170')" prop="modelResultTime" align="center" min-width="90px"> <!-- 获取模型结果的时间 -->
        <template slot-scope="{ row }">
      ...
      <el-table-column
        v-if="!isModelResultNext && permission.isAdmin"
        key="modelResultNext"
        :label="$t('collect.collect168')"
        prop="modelResultNext"
        align="center"
        min-width="80px"
      > <!-- 准入期 -->
        <template slot-scope="{ row }">
      ...
      <el-table-column
        ...
        key="releasePayCompany"
        :label="$t('borrow.borrow35')"
        prop="releasePayCompany"
        align="center"
        min-width="70px"
      > <!-- 支付公司 -->
        <template slot-scope="{ row }">
      ...
      <el-table-column v-if="permission.isAdmin" key="releaseResultResponeId" :label="$t('collect.collect185')" prop="releaseResultResponeId" align="center" min-width="90px" /> <!-- 放款结果响应编号 -->
      ...
      <el-table-column key="repayStartTime" :label="$t('collect.collect196')" align="center" min-width="90px"> <!-- 开始还款时间 -->
        <template slot-scope="{ row }">
...
export default {
  ...
  methods: {
    ...
    async getUserBorrowData() {
      const res = await getUserBorrowData({ idRes: this.userIdRes })
      ...
      this.isReleaseResultSucceed = res.data.every(item => !item.releaseResultSucceed)
