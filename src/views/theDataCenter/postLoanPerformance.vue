<template>
  <div class="main-box" style="padding-bottom: 80px;">
    <!-- 运营管理 -- api转换 -->
    <el-form ref="listQueryForm" :model="listQuery" size="mini" label-width="160px" inline>
...
export default {
  ...
  mounted() {
    setTimeout(() => {
      this.getTableList(this.marketTypeOptions[0].marketType)
    }, 100)
  },
  ...
  methods: {
    ...
    async getTableList(item) {
      ...
      if (item) this.listQuery.marketType = item
    ...
    resetList() {
      this.listQuery = {
        marketType: this.marketTypeOptions[0].marketType,
        month: ''
      }
      this.getTableList()
    }
